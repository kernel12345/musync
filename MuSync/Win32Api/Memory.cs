...
internal static class Memory
{
    ...
    internal static int ModuleCacheCount => ModuleCache.Count;
...
internal sealed partial class ProcessMemory(nint process) : IDisposable
{
    ...
    {
        ProcessId = processId;
        // OpenProcess 返回空句柄且错误码为 5（拒绝访问）时，明确抛出以便上层提示管理员权限问题
        if (Handle == IntPtr.Zero && Marshal.GetLastWin32Error() == 5)
        {
            throw new UnauthorizedAccessException(
                "拒绝访问目标进程内存 (ERROR_ACCESS_DENIED)，可能需要以管理员身份运行。");
        }
    }
    public void Dispose()
    {
