...
internal static class ImageCacheManager
{
    ...
    public static async Task<Image?> LoadImageAsync(string cacheKey, string imageUrl)
    {
        ...
        // 本地文件封面（如汽水音乐经 SMTC 取到后落盘的缩略图）：文件 IO + GDI+ 解码移到后台线程，避免阻塞 UI
        var localPath = TryGetLocalImagePath(imageUrl);
        if (localPath != null)
        {
            return await Task.Run(() => LoadFromLocalFile(cacheKey, localPath));
        }
        ...
        for (var retry = 0; retry < maxRetries; retry++)
        {
            ...
            try
            {
                ...
                // 图片解码（GDI+ Image.FromStream）是 CPU 密集操作，移到后台线程避免阻塞 UI
                Image? image;
                long imageSize;
                try
                {
                    (image, imageSize) = await Task.Run(() =>
                    {
                        var ms = new MemoryStream(downloadedBytes!);
                        var img = Image.FromStream(ms);
                        var size = MemoryPressureMonitor.EstimateImageSize(img);
                        return (img, size);
                    });
    ...
    /// <summary>识别 file:// URI 或本地绝对路径，返回实际文件路径；远程 URL 返回 null。</summary>
    private static string? TryGetLocalImagePath(string url)
    {
        ...
        if (!url.StartsWith("http://", StringComparison.OrdinalIgnoreCase) &&
            ...
            Path.IsPathRooted(url) && File.Exists(url))
        {
            return url;
        }
        return null;
    ...
    private static Image? LoadFromLocalFile(string cacheKey, string path)
    {
        try
        {
            ...
            {
                Logger.Diagnose($"Local cover is not a valid image for key: {cacheKey}.");
                return null;
            }
            // GDI+ 对部分格式要求支撑流保持存活，MemoryStream 不主动释放（与 HTTP 路径一致）
            ...
            lock (LockObject)
            {
                ...
                {
                    var lruNode = LruList.Last;
                    var evictedSize = MemoryPressureMonitor.EstimateImageSize(lruNode.Value.Image);
                    RemoveFromCacheInternal(lruNode.Value.Key);
                    MemoryPressureMonitor.UnregisterImageSize(evictedSize);
                }
                var newItem = new CacheItem(cacheKey, image);
                var newNode = new LinkedListNode<CacheItem>(newItem);
            ...
            Logger.Diagnose($"Loaded local cover into cache for key: {cacheKey}.");
            return image;
        }
        catch (Exception ex)
        {
            Logger.Diagnose($"Failed to load local cover for key {cacheKey}: {ex.Message}");
            return null;
        }
    }
